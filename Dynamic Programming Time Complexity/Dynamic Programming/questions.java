public class questions {
        ...
        for(int n = 0; n<=N; n++){
            for(int m = 0; m<= M; m++){
                ...
                if(s1.charAt(n-1) == s2.charAt(m-1))
                    ...
                    dp[n][m] = Math.min(Math.min(insert+cost[1], replace+cost[2]), delete+cost[3]);
            }
        }
        return dp[N][M];
    }
    public int minCostToConvert_DP(String word1, String word2) {
        int n = word1.length(), m = word2.length();
    ...
    public static int minDeleteToMakePlaindrome(String str) {
        int n = str.length();
        int[][] dp = new int[n][n];
